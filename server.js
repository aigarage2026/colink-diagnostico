...
app.get('/api/health', (req, res) => {
  ...
  let publicFiles = [];
  try { publicFiles = fs.readdirSync(publicDir); } catch(e) { publicFiles = ['ERROR: ' + e.message]; }
  res.json({ 
    status: 'ok', 
...
const fs = require('fs');
// Detect public directory - try multiple paths
const possibleDirs = [
  path.join(__dirname, 'public'),   // standard: /app/public
  __dirname,                          // flat: /app (files at root)
...
// Last resort: list what IS in /app
if (!PUBLIC_DIR) {
  let listing = 'NOT FOUND. Dirs tried: ' + possibleDirs.join(', ') + '. ';
  try { listing += 'Files in __dirname: ' + fs.readdirSync(__dirname).join(', '); } catch(e) {}
  try { listing += ' | Files in cwd: ' + fs.readdirSync(process.cwd()).join(', '); } catch(e) {}
  PUBLIC_DIR = path.join(__dirname, 'public');
  console.error('index.html not found!', listing);
}
...
app.get('*', (req, res) => {
  ...
  if (!fs.existsSync(indexPath)) {
    let dbg = 'index.html not found at: ' + indexPath;
    try { dbg += ' | /app: ' + fs.readdirSync('/app').join(', '); } catch(e) {}
    try { dbg += ' | /app/public: ' + fs.readdirSync('/app/public').join(', '); } catch(e) {}
    return res.status(500).send(dbg);
  }
