...
let PUBLIC_DIR = possibleDirs.find(d => {
  try { return fs.existsSync(path.join(d, 'index.html')); } catch(e) { return false; }
}) || path.join(__dirname, 'public');

console.log('__dirname:', __dirname);
console.log('cwd:', process.cwd());
console.log('Public dir resolved:', PUBLIC_DIR);
...
app.get('*', (req, res) => {
  ...
  if (!fs.existsSync(indexPath)) {
    return res.status(500).send('Cannot find index.html. PUBLIC_DIR=' + PUBLIC_DIR + ' cwd=' + process.cwd() + ' __dirname=' + __dirname);
  }
