...
// Explicit routes for root and admin
app.get('/', (req, res) => {
...
function dbReady(req, res, next) {
  if (!db) return res.status(503).json({ error: 'Banco de dados ainda conectando. Aguarde alguns segundos e tente novamente.', retry: true });
  next();
}
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  // Connect DB after server is up — don't block startup
  connectDB().catch(err => {
