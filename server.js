...
// relatorios.html is served by express.static above
// Explicit fallback in case static misses it
app.get('/relatorios.html', (req, res, next) => {
  const file = path.join(__dirname, 'public', 'relatorios.html');
  const fs = require('fs');
  if (!fs.existsSync(file)) {
    console.error('relatorios.html NOT FOUND at:', file);
    return res.status(404).send('relatorios.html not found. Check that public/relatorios.html exists.');
  }
